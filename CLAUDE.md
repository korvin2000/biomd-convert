...
- **A new hook ships disabled.** `enabledByDefault` is pinned to `table.classify` + `table.records` by `src/llm/plugins/plugins.test.ts`; `--llm assist` with nothing enabled must be byte-identical to `--llm off` and is verified as such. Existing hooks: `table.classify`, `table.records` (wired) and `text.segment` (migrated, no escalation site, inert and reported as such).
