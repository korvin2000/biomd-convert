...
| **A line holding two sentences is prose**, so `TEXT_LIST.accept` can veto a wrong `LIST` on shape | It refused `Том VII Ура! Каникулы! (2000)` — an exclamation inside a title is not a sentence boundary — and with it the very list the hook exists for. Initials make the naive form worse still (`J. S. Bach - Allemande`). It was re-deriving §15.2's killed discriminator one layer down: **a check strong enough to catch the bad answer would have decided deterministically, and then the hook cannot exist.** §59.1 | one corpus run with `--hooks text.list`, read the refusal tally |
| **End-of-line punctuation separates a series from an enumeration** — a line ending `,`/`;`/`.` is a member of one punctuated sentence, not an entry. Would have split `kiselev`/`jovicic`/`xtra_rodrigo` from `new_lendle2`/`new_bach` exactly | **Swept over all 28 references**: list items end in `,` six times, `;` three, `.` three. `xtra_alexandro` writes `- Prelude No. 13;` / `- Prelude No. 14;` as list items — the same shape `new_lendle2` keeps as prose. The references disagree with each other, which is a fact about the references and not a signal. §59.1 | one 15-line sweep over `fixtures/out/*.bio.md` |
| **An aligned bold label outside a grid is the non-firing control** for §59.2's lifted-band rule | It is not: a short shouted line outside a grid is recovered as a *heading*, and §3.1's author ruling drops a recovered label's alignment. The control proves the heading path, not the align path. A credit line is the shape that actually takes a wrapper. §59.2 | one `convert` on a three-block probe page |
| **Word count separates a section label from a lead-in sentence.** The corpus's labels run 1-4 words and its lead-ins 6-15, so a cap at 5 promotes every true positive and no sentence -- a clean margin over 28 documents | `recovery.test.ts` already asserted that `Формулируя цели Сеговия писал:` stays plain, and that is a **four-word sentence**. The cap was fitted to the corpus and would have been wrong on the other ~987 pages. What survives instead is the difference between the brief's own terms: a shout and a section-opening word are evidence only a label has; a trailing colon, shortness and standing clear are satisfied by prose. §60.5 | one `npm test` -- the contract predated the rule |
| **`williams2`'s `Em\*\*` is asterisk handling.** `TODO_Rules.md` §4 reaffirms it by name, so the escape logic must be dropping stars | Reopened and re-measured rather than re-argued. Every literal asterisk in all 28 sources already survives and escapes; a probe page confirms `Em**`, `Em*`, `Em***`, a lone `*` and `**double**` all escape correctly. The cell is `<i>&nbsp;</i>` -- an empty italic, **no asterisk in the source** -- and there is exactly **one** empty inline mark in the whole corpus. It is one cell and an author question, not a rule. §60.1 | one probe page + one regex sweep over `fixtures/html` |
