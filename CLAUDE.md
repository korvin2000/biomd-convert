...
- **Reference-silent constructs** (`src/eval/reference-silent.ts`, contract `reference-silent.test.ts`): a reference written before a construct existed cannot adjudicate it, so L1 and L2 ignore it **per document and per construct while that reference contains none of it** -- one occurrence anywhere in the reference turns full comparison back on for that document. Symmetric (both sides or neither), hides an absence and never a disagreement, self-retiring: nothing to undo when references gain the construct. Two entries: `anchor`, a directive (PROGRESS §52.4), and `==highlight==`, an inline mark (§56.4) -- the inline one folds the marks out of **both** sides' text, so a disagreement about *which* run is marked still reports. Adding one is an invariant-2 instrument change: declare it, justify it as truthfulness, re-baseline both sides.
...
- **Tie-break for verdict 4 when the author has no discriminator either** (*ruled 2026-08-10*): take the reading that leaves the **rule system** least contradictory. Fewest inconsistencies across the corpus, therefore most generalizable to the other ~987 pages, and least metric damage -- in that order. Two references disagreeing on one shape is a fact about the references, not a rule; obeying the majority reading and recording the minority as a known, named divergence beats a special case that only one document can ever justify. The single-record table is the worked example (PROGRESS §36.5): 3 documents to 1, and the 1 is recorded, not patched.
...
- Keep production hooks and L4 judge strictly separate by models/prompts/purpose. Adjudicate with LLM off for comparability; measure LLM-on delta separately and label it.
- **Architecture and authoring contract -> `biomd-convert/docs/LLM-HOOKS.md`.** A hook is a plugin directory (`src/llm/plugins/<name>/` = `hook.ts` + `prompts/*.md` + `hook.test.ts`), discovered from the filesystem; there is no list of hook names anywhere and no CLI flag per hook. `convert-core` declares a `DecisionPoint` beside each abstaining rule (`src/convert-core/decisions.ts`) carrying the **acceptance check**, which is the last word and runs on the deterministic side. Prompts are external, hashed, and part of cache identity. Discover/inspect/exercise with `biomd hooks list | show | test | cache-clear`.
- **Three entry tests, all required, before writing one:** name the abstention (the state where the rule produced *no* answer); name the acceptance check (and reject the circular case where a check strong enough to catch the bad answer would have decided deterministically); prefer visible failure (a wrong caption or a corrupted word reads as fact — a wrong heading does not).
- **Every hook ships disabled, and the default-enabled set is empty** — `enabledByDefault: false` on every plugin, pinned by `src/llm/plugins/plugins.test.ts`. `llm.enabled: true` alone runs no hook; `--llm assist` with nothing named is byte-identical to `--llm off`, unconditionally. Enabling one is a conversion change: name it in `llm.hooks.enable` (config) or `--hooks <ids>` (one run), and measure it against the LLM-off baseline. An unknown id is a startup error. Existing hooks: `table.classify`, `table.records`, `text.list` (wired) and `text.segment` (migrated, no escalation site, inert and reported as such). Hook-on runs go through `bench/biomd.llm.config.json` (`bench/out-llm/`), never `bench/biomd.config.json`.
