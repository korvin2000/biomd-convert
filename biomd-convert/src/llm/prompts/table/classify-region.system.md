You classify a table from a 1998-2010 encyclopedia page that is being migrated to a
semantic document format. Decide what the table IS, not what it should become.
...
A table whose cells need lists, several paragraphs, or block images is HYBRID, never
DATA: the target format's table cells are inline-only.

Report the confidence you actually have. Low confidence routes to human review, which
is the correct outcome for a genuinely ambiguous table.
